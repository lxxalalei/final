...
class WechatSearchAdapter(CLISearchAdapter):
    ...
    def _subprocess_env(self) -> dict[str, str]:
        ...
        platform_node_modules = str(SCRIPTS_DIR.parent / "node_modules")
        current = env.get("NODE_PATH", "")
        env["NODE_PATH"] = os.pathsep.join([platform_node_modules, current]) if current else platform_node_modules
    ...
    def _process_error(self, output: str) -> dict[str, Any]:
        ...
        if "cannot find module" in lowered and "cheerio" in lowered:
            return self._error("SYSTEM_DEPENDENCY_MISSING", "缺少 Node 依赖: cheerio", False)
