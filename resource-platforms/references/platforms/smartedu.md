...
python3 scripts/smartedu/smartedu_download.py download \
...
```bash
# 指定输出目录
python3 scripts/smartedu/smartedu_download.py \
  "https://..." --output-dir ./downloads

# 只下载特定格式
python3 scripts/smartedu/smartedu_download.py download "https://..." --formats pdf,m3u8

# 指定视频下载并发数
python3 scripts/smartedu/smartedu_download.py download "https://..." --video-concurrency 5

# 查看所有可用格式
python3 scripts/smartedu/smartedu_download.py formats "https://basic.smartedu.cn/qualityCourse?courseId=xxx"
```
...
```bash
# 第一步：同步教材索引（首次运行需要）
python3 scripts/smartedu/smartedu_tch_material.py \
...
```bash
python3 scripts/smartedu/smartedu_resources.py \
  textbook-candidates \
  --grade "三年级" --subject "数学" --version "人教版" --volume "上册" \
