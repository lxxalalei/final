...
python3 scripts/zhihu/zhihu_search.py search "科普知识" --max 20
```

### download(candidate) → result

已实现。脚本路径：`scripts/zhihu/zhihu_dl.py`
...
```bash
# 下载问答页面（导出为 Markdown）
python3 scripts/zhihu/zhihu_dl.py download \
...
python3 scripts/zhihu/zhihu_search.py search "关键词" -o output.json
```
