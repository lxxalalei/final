...
> - 各平台下载方法的权威定义在各 `<平台>.md`（同目录）中，本文档与之保持一致。
...
> **本系统权威实现**：`scripts/smartedu/smartedu_download.py`（`download_m3u8_video()` 函数）。
...
# 指定只下载视频（m3u8），并设置并发数
python3 scripts/smartedu/smartedu_download.py download "https://..." \
...
> **适用平台**：douyin（抖音）。
> **本系统权威实现**：`scripts/douyin/douyin_dl.py`（`F2Engine` 类）。
...
# 签名失效时降级到 CDP 浏览器模式
python3 scripts/douyin/douyin_dl.py --cdp http://127.0.0.1:9222 download <URL>
```
