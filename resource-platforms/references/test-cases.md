...
- 教材栏目应仍标记为 `known_skill=smartedu`，并通过 `resource_family=教材`、`internal_adapter=tchMaterial` 表示站内教材资源分支。
...
`candidates-from-detail` 输出应能直接交给 `resource-selector` 进行评分和选择。
