...
def _platforms_base_dir() -> Path:
    """返回 platforms/ 目录的绝对路径。"""
    # shared/ 的同级目录下有 platforms/
...
class CLIBasedPlatformSkill(PlatformSkill):
    ...
    **向后兼容**：所有新参数都有默认值，现有子类无需修改 ``__init__`` 签名即可获得新功能。
    ...
    def _discover_script(self, pattern: str) -> Path | None:
        """按命名约定发现脚本。"""
        scripts_dir = base / self.platform_name / "scripts" if (base := _platforms_base_dir()) else None
        if scripts_dir is None:
            scripts_dir = _platforms_base_dir() / self.platform_name / "scripts"
