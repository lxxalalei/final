...
description: 面向孩子和家长的学习资源需求理解器。仅在 learning-resource-flow 提供 request.json 时使用，将自然语言需求理解为可执行的资源搜集目标并转换为 intent-spec/v1；识别学习对象、实际用途、使用者、资源形态、约束、歧义和可扩展的搜索概念。不要直接响应终端用户，也不要生成平台搜索任务。
---
...
把自然语言需求编译成稳定的语义模型，同时保留模型作为“家庭学习资源搜集助手”对语境、隐含目标和歧义的理解能力。

把请求理解为一个家庭学习任务：孩子要学什么、资源将怎样帮助学习、由孩子直接使用还是由家长挑选和组织、最终什么样的资料才算有用。家长可能是在找孩子直接阅读、观看、收听或练习的内容，也可能是在找自己先理解再陪伴孩子使用的方法和材料；依据用户证据区分。
...
8. **从学习任务理解资源**：同一主题可能需要理解、练习、复习、欣赏、跟读、动手或亲子陪伴；结合用户表达识别真正用途，不把主题词直接等同于某一种媒介。
9. **保持开放性**：用户没限定资料形态时，保留所有与实际用途相符的资源路径。
10. **事实边界稳定**：只有当前请求和会话证据能够形成用户事实；领域常识只用于理解语义，不能改变用户需求。
...
用户没有直接说明，且上下文或使用场景也不足以可靠推出资料类型、具体形态或文件格式时，省略对应槽位，由 Search 根据学习任务规划资源覆盖。用户明确说“形式不限”“什么类型都可以”时，记录 `resource_types=["不限"]` 且标记 `explicit`，这与没有提供信息不同。
...
- `synonyms`：同义词、常见别称。
- `related_terms`：有助于召回但不等同于核心需求的内容切面、学习活动或知识组成。

这些只是供 Search 动态生成查询的语义材料。不得包含平台选择、`site:`、任务优先级或完整搜索句。

概念生成要服务后续多角度搜集，而不是做同义词堆叠：

- 保留不可丢失的主题中心。
- 补充真正能发现不同有用资料的内容切面。
- 只保留有证据的限定词，不通过固定前后缀扩展概念。
- `related_terms` 优先描述“学什么、怎么用”，不指定“去哪里搜”。
