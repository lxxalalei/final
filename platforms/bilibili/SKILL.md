---
name: bilibili
description: B站资源站点。负责B站视频搜索、信息获取和下载，实现平台 Skill 标准接口。接口契约见 shared/schemas/platform-search-contract.md 和 platform-download-contract.md。集成 bilibili-api-python 提供搜索、字幕、排行榜等能力。
---
...
```bash
python3 platforms/bilibili/scripts/bili_api.py search \
...
```bash
python3 platforms/bilibili/scripts/bilibili_dl.py search \
...
```bash
# 单个视频
python3 platforms/bilibili/scripts/bilibili_dl.py download BV1xxx -o ./downloads/

# 批量下载
python3 platforms/bilibili/scripts/bilibili_dl.py batch list.json -o ./downloads/

# UP主全部视频
python3 platforms/bilibili/scripts/bilibili_user_dl.py <空间URL或UID> -o ./downloads/
```
...
```bash
# 搜索视频（无需登录）
python3 platforms/bilibili/scripts/bili_api.py search "小学数学" --max 10

# 视频详情
python3 platforms/bilibili/scripts/bili_api.py video BV1xxx

# 字幕获取（plain / srt 格式，部分视频需登录）
python3 platforms/bilibili/scripts/bili_api.py subtitle BV1xxx --format srt -o sub.srt

# 全站排行榜
python3 platforms/bilibili/scripts/bili_api.py rank --max 10

# UP主视频列表
python3 platforms/bilibili/scripts/bili_api.py user-videos 946974 --max 10
```
