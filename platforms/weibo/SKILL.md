---
name: weibo
description: 微博资源站点。负责微博用户图文搜索和下载，实现平台 Skill 标准接口。
---
...
python3 platforms/weibo/scripts/weibo_dl.py search "教育方法" --max 20 --cookie weibo_cookies.txt

# 搜索结果输出到文件
python3 platforms/weibo/scripts/weibo_dl.py search "科普知识" --max 20 --cookie weibo_cookies.txt -o candidates.json
```
...
```bash
# 按用户主页 URL 下载
python3 platforms/weibo/scripts/weibo_dl.py download https://weibo.com/u/1669879400 -o ./downloads/

# 按用户名下载
python3 platforms/weibo/scripts/weibo_dl.py download https://weibo.com/n/用户名 -o ./downloads/ --max-pages 5

# 按 UID 下载
python3 platforms/weibo/scripts/weibo_dl.py download --uid 1669879400 -o ./downloads/
